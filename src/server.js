...
const STATE_DIR = process.env.OPENCLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim() || path.join(os.homedir(), ".openclaw");
const WORKSPACE_DIR = process.env.OPENCLAW_WORKSPACE_DIR?.trim() || process.env.CLAWDBOT_WORKSPACE_DIR?.trim() || path.join(STATE_DIR, "workspace");
...
const OPENCLAW_ENTRY = process.env.OPENCLAW_ENTRY?.trim() || process.env.CLAWDBOT_ENTRY?.trim() || "/clawdbot/dist/entry.js";
...
<body>
  ...
  <div class="card">
    ...
    <div style="margin-top: 0.75rem">
      <a href="/clawdbot" target="_blank">Open OpenClaw UI</a>
      &nbsp;|&nbsp;
...
app.post("/setup/api/run", requireSetupAuth, async (req, res) => {
  ...
  if (ok) {
    ...
    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "gateway.auth.token", GATEWAY_TOKEN]));
...
const server = app.listen(PORT, "0.0.0.0", () => {
  ...
  if (isConfigured()) {
    ...
    ensureGatewayRunning()
      .then(() => console.log("[wrapper] gateway started successfully"))
